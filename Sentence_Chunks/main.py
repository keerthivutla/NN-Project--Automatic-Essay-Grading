...
x_data,y_data,vocab,essay_sentences = get_data()
...
x_valid = x_data[ts:]
y_valid = y_data[ts:]
valid_sentences = essay_sentences[ts : ]

for i in range(len(valid_sentences))
	max_sentences = max(len(valid_sentences[i]),max_sentences)
...
ct = 0
nChunks = 2
newEssays = np.zeros(n_chunks*(len(valid_sentences)),maxSentenceLength,maxWordLength)

for essay in valid_sentences:
	numSents = len(valid_sentences[i]) / nChunks
	for j in range(nChunks):
		new_essays[ct] = essay[ j * numSents : (j+1) * numSents]
		ct+=1

print("New Essays Shape",newEssays.shape)
print("Old Essay Shape",x_valid.shape)
print(len(valid_sentences[0]))
